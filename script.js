...
function initializeApp() {
    ...
    document.getElementById('proposta-data').value = today.toISOString().split('T')[0];
    document.getElementById('data-assinatura').value = today.toISOString().split('T')[0];
    ...
    // Definir texto padrão do termo
    document.getElementById('termo-texto').value = `Declaramos, para os devidos fins, que concordamos com a proposta enviada em ${formatDate(today)} Nº ${proposalNumber}`;
}
...
function setDefaultDates() {
    ...
    // Atualizar texto do termo com a data atual
    const proposalNumber = document.getElementById('proposta-num').value;
    document.getElementById('termo-texto').value = `Declaramos, para os devidos fins, que concordamos com a proposta enviada em ${formattedDate} Nº ${proposalNumber}`;
}
...
function saveProposal() {
    const proposal = {
        ...
        termoTexto: document.getElementById('termo-texto').value,
    ...
    container.innerHTML = `
        <div class="proposal-document pdf-export" style="background: white; padding: 15px; font-family: 'Arial', sans-serif; color: #333; line-height: 1.2; font-size: 9px;">
            ...
            <div style="display: flex; gap: 15px; margin-bottom: 10px;">
                ...
                <div style="flex: 1; background: #f8f9fa; padding: 8px; border-radius: 4px; border-left: 3px solid #ffc107;">
                    <h3 style="color: #2c3e50; font-size: 10px; font-weight: 600; margin: 0 0 5px 0;">ACEITE</h3>
                    <p style="margin: 2px 0; font-size: 8px; color: #555; line-height: 1.3;">${proposal.termoTexto || 'Declaramos que concordamos com a proposta enviada e aceitamos todas as condições apresentadas.'}</p>
...
async function generatePDF() {
    try {
        ...
        // Configurações otimizadas para PDF profissional
        const canvas = await html2canvas(element, {
            scale: 2, // Resolução balanceada
            useCORS: true,
            allowTaint: true,
        ...
        const pdf = new jsPDF('p', 'mm', 'a4'); // 'p' = portrait (em pé), 'l' = landscape (deitado)
...
function loadSavedProposal() {
    ...
    if (saved) {
        ...
        if (currentProposal.termoTexto) document.getElementById('termo-texto').value = currentProposal.termoTexto;
